...
Seance 11 - TP Tests Donnees & Modele
    Evalue un modele du registry avec `mlflow.models.evaluate` puis applique
    une porte qualite avec `mlflow.validate_evaluation_results`.
"""
...
from mlproject.config import (
    ...
    EVAL_F1_MIN,
    EVAL_ROC_AUC_MIN,
    MLFLOW_EXPERIMENT_EVAL,
    MLFLOW_EXPERIMENT_EVAL_DESCRIPTION,
    MLFLOW_EXPERIMENT_EVAL_TAGS,
    MODEL_NAME,
    TARGET,
)
...
def build_thresholds() -> dict[str, MetricThreshold]:
    """Construire les seuils minimaux de validation."""
    return {
...
def evaluate_model(model_uri: str | None = None, validate: bool = True):
    ...
    setup_experiment(
        ...
        description=MLFLOW_EXPERIMENT_EVAL_DESCRIPTION,
        tags=MLFLOW_EXPERIMENT_EVAL_TAGS,
    )
...
def main() -> None:
    ...
    try:
        evaluate_model(model_uri=args.model_uri or None, validate=args.validate)
