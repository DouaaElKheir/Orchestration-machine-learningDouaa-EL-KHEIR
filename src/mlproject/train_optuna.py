...
from mlproject.config import (
    MLFLOW_EXPERIMENT_OPTUNA,
    MLFLOW_EXPERIMENT_OPTUNA_DESCRIPTION,
    MLFLOW_EXPERIMENT_OPTUNA_TAGS,
    MODEL_DIR,
    MODEL_NAME,
    RANDOM_STATE,
)
...
def describe_registered_version(
    ...
    description = (
        f"Modele {result.spec.name} optimise par Optuna (sampler TPE, "
...
def optimize(n_trials: int = 30, cv: int = 5, use_mlflow: bool = True) -> list[FamilyResult]:
    ...
    if use_mlflow:
        setup_experiment(
            experiment_name=MLFLOW_EXPERIMENT_OPTUNA,
            description=MLFLOW_EXPERIMENT_OPTUNA_DESCRIPTION,
            tags=MLFLOW_EXPERIMENT_OPTUNA_TAGS,
        )
