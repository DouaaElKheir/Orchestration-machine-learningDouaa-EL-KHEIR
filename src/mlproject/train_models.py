...
from mlproject.config import (
    MLFLOW_EXPERIMENT_MODELS,
    MLFLOW_EXPERIMENT_MODELS_DESCRIPTION,
    MLFLOW_EXPERIMENT_MODELS_TAGS,
    MODEL_DIR,
    MODEL_NAME,
    RANDOM_STATE,
)
...
def describe_registered_version(
    ...
    description = (
        ...
        f"Dataset : Adult Income (prediction revenu >50K).\n"
...
def train_all(
    ...
    if use_mlflow:
        setup_experiment(
            experiment_name=MLFLOW_EXPERIMENT_MODELS,
            description=MLFLOW_EXPERIMENT_MODELS_DESCRIPTION,
            tags=MLFLOW_EXPERIMENT_MODELS_TAGS,
        )
...
def main() -> None:
    ...
    results = train_all(cv=args.cv, scoring=args.scoring, use_mlflow=args.use_mlflow)
    print("\n=== Comparaison des modeles (Adult Income) ===")
    for rank, result in enumerate(results, start=1):
        print(
            f"{rank}. {result.name:15s}  "
            f"roc_auc={result.roc_auc:.3f}  f1={result.f1:.3f}  "
            f"cv_{args.scoring}={result.cv_score:.3f}"
        )
